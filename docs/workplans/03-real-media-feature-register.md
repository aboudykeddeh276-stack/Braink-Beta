...
2. **Scored by an LLM reading a synopsis/description** (i.e., an actual model call producing a style/emotion rating). This is legitimate but must be labeled `CONCEPTUAL`/model-derived, not `VERIFIED`, in the same tagging scheme the rest of this repo uses — a language model's rating is an opinion, not a measurement.
