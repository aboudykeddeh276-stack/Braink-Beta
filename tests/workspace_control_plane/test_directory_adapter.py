...
def test_create_then_get_round_trips():
    ...
    assert adapter.get_user("new@example.com").given_name == "New"
...
def test_create_duplicate_user_raises():
    ...
    with pytest.raises(ValueError):
        adapter.create_user(CanonicalUser(primary_email="dup@example.com"))
...
def test_update_unknown_user_raises():
    ...
    with pytest.raises(KeyError):
        adapter.update_user("ghost@example.com", {"given_name": "X"})
...
def test_suspend_user_sets_state_and_flag():
    ...
    result = adapter.suspend_user("s@example.com")
...
def test_delete_user_removes_it():
    ...
    adapter.delete_user("d@example.com")

    assert adapter.get_user("d@example.com") is None
...
def test_get_user_returns_a_copy_not_internal_state():
    ...
    snapshot = adapter.get_user("c@example.com")
    snapshot.given_name = "Mutated"

    assert adapter.get_user("c@example.com").given_name == "Orig"
